...
export default function ReadPanel({ client }) {
	...
	const getItem = async (e) => { e?.preventDefault(); try { const tx = await client.get_item({ item_id: BigInt(getItemId) }, { simulate: true }); setGetItemOut(tx.result); } catch (e) { setGetItemOut({ error: String(e) }); } }
	const getItemCount = async () => { try { const tx = await client.get_item_count({ simulate: true }); setItemCountOut(tx.result); } catch (e) { setItemCountOut({ error: String(e) }); } }
	const getRentalCount = async () => { try { const tx = await client.get_rental_count({ simulate: true }); setRentalCountOut(tx.result); } catch (e) { setRentalCountOut({ error: String(e) }); } }
	const getRental = async (e) => { e?.preventDefault(); try { const tx = await client.get_rental({ rental_id: BigInt(rentalId) }, { simulate: true }); setGetRentalOut(tx.result); } catch (e) { setGetRentalOut({ error: String(e) }); } }
	const getItemRentals = async (e) => { e?.preventDefault(); try { const tx = await client.get_item_rentals({ item_id: BigInt(itemRentalsItemId) }, { simulate: true }); setItemRentalsOut(tx.result); } catch (e) { setItemRentalsOut({ error: String(e) }); } }
	const getUserRentals = async (e) => { e?.preventDefault(); try { const tx = await client.get_user_rentals({ user: userAddress.trim() }, { simulate: true }); setUserRentalsOut(tx.result); } catch (e) { setUserRentalsOut({ error: String(e) }); } }
	const isOverdue = async (e) => { e?.preventDefault(); try { const tx = await client.is_overdue({ rental_id: BigInt(isOverdueRentalId) }, { simulate: true }); setIsOverdueOut(tx.result); } catch (e) { setIsOverdueOut({ error: String(e) }); } }

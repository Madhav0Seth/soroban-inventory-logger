...
export async function connectFreighter() {
	...
	if (!access || access.error) throw new Error(access?.error || 'Access to Freighter wallet was denied')
	const addr = await getAddress()
	if (addr.error || !addr.address) throw new Error(addr.error || 'Failed to get address from Freighter')
	return addr.address
}
